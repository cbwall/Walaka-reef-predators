...
pacman::p_load("knitr", "ggord", "MASS", "tidyverse", "plyr", "dplyr", "gridExtra", 
               "cowplot", "ggplot2", "R2jags", "MixSIAR", "remotes", 'SIBER', )
```
...
# boxplots high residency (Residency_index ≥ 0.1, ∼inshore) vs low residency (Residency_index < 0.1, ∼offshore) 
boxplot(d15N_SAA ~ Residency, data = AA.indic,
        col=c("lightgreen","lightcoral"),
        ylab="δ15NSAA", xlab="Residency")
boxplot(TPGlx_Phe ~ Residency, data = AA.indic,
        col=c("lightgreen","lightcoral"),
        ylab="TPGlx_Phe", xlab="Residency")
boxplot(TPAla_Phe ~ Residency, data = AA.indic,
        col=c("lightgreen","lightcoral"),
        ylab="TPAla_Phe", xlab="Residency")
boxplot(PTS ~ Residency, data = AA.indic,
        col=c("lightgreen","lightcoral"),
        ylab="PTS", xlab="Residency")

# ANOVA
model <- aov(d15N_SAA ~ Residency, data = AA.indic)
print(summary(model))
model <- aov(TPGlx_Phe ~ Residency, data = AA.indic)
print(summary(model))
model <- aov(TPAla_Phe ~ Residency, data = AA.indic)
print(summary(model))
model <- aov(PTS ~ Residency, data = AA.indic)
print(summary(model))

# correlations with Residency_index
# list of variables to analyze
vars <- c("d15N_SAA", "TPGlx_Phe", "TPAla_Phe", "PTS")
...
# plot using ggplot2
  p <- ggplot(AA.indic, aes(x = Residency.index, y = .data[[var]])) +
...
dev.copy(pdf, "figures/Walak_Fig.mixsiar.3sour.EAAn.pdf", width = 6, height = 5)
dev.off()
```
...
dev.copy(pdf, "figures/Walak_Fig.mixsiar.2sour.EAAn.pdf", width = 4, height = 5)
dev.off()
```
...
```{r res by inshore proportion EAA, fig.cap="**Figure 7B.** Relationship between the proportion of inshore carbon sources utilized versus and the predator residency index. The correlation is significant (ANOVA, f = 20.943, p <0.001)"}
...
Inshore.prop.plant
ggsave("figures/Walak_Inshore.prop.EAAn.pdf", width=5, height=5)
```
