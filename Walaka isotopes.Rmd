...
### Data 
Data here consists of bulk and amino acid δ^13^C and δ^15^N isotope valyes for Giant trevally and Galapagos sharks from the Northwestern Hawaiian Islands, at French Frigate Shoals atoll. We also include data used to calculate residence index using telemetry, as well as info on tagging dates and consumer fork length. Lastly, we incorporate the data from Skinner et al. (2021) reporting δ^13^C isotope values for essential amino acids (EAAs) collected inshore and offshore of coral reefs in the Maldives. These are used as trianing data to assign fish consumers and determine the proportion support of foodweb support to consumers.  
...
write.csv(AllProdPredict, "output/Walak_LDA_coords.csv", row.names = FALSE)
```

CLassidy consumers (giant trevally or Galapagos sharks) as belonging to the three source groups. We show the classification gives about 1/2 of the trevally as inshore vs. offshore, with 3 of 4 sharks being inshore.
...
write.csv(cons.predict.data, "output/Walak_LDA_info.csv", row.names=FALSE)
...
```{r LDA figure, fig.cap="**Figure 6.** Figure 6. Linear discriminate analysis (LDA) of δ^13^C~EAA~ fingerprints from reef-associated plankton, pelagic plankton and coral compared with those of predators. Ellipses denote 95% confidence intervals for each source group.", results='hide'}
#### plot it
...
write.csv(Mix.df.EAA.consm, "output/Walak_Mix.df.EAAn.consumer.csv", row.names = FALSE)
...
cons.EAAn <- load_mix_data(filename = "output/Walak_Mix.df.EAAn.consumer.csv",
...
```{r MixSIAR model Jags, cache=TRUE}
################## ################## ################# 
...
saveRDS(consm.jags.mod.EAAn, file = "output/MixSIAR_models/EAAn_SampleID_model/MixSIAR_Model_SampleID_rjags.rds")

# code below will read back in the RDS object
consm.jags.mod.EAAn <- readRDS(file = "output/MixSIAR_models/EAAn_SampleID_model/MixSIAR_Model_SampleID_rjags.rds")
```

Generate plots for MixSIAR diagnostics and perform source aggregation.
```{r MixSIAR diagnostics, eval=FALSE}
...
```{r 3 sources MixSIAR, results='hide', fig.cap="**Figure 7a.** Results of the MixSIAR Bayesian mixing model using δ13CEAA fingerprints. Proportional contribution of pelagic plankton, reef-associated plankton and corals to giant trevally and Galapagos sharks."}
################## ready to plot ################## 
...
write.csv(Mix.samp.EAAn.2, "output/Walak_Mix.samp.EAAn.2.csv")
...
# group mean model
Inshore.mod<-lm(Mean~Group, data=Inshore.df) #p=0.974, no difference in % inshore by group
Group.model<-anova(Inshore.mod)

# residency by mean model
Res.Inshore.mod<-lm(Mean~Residency.index, data=Inshore.df) #p=0.0145
Res.sum<-summary(Res.Inshore.mod)
anova(Res.Inshore.mod)
```

#### Residency by proportion of inshore foodweb support. 

```{r residency by inshore proportion EAA, *fig.cap=**Figure 7B.* Relationship between the proportion of inshore carbon sources utilized versus and the predator residency index. The correlation is significant (ANOVA, f = 7.64, p = 0.015)"}
...
Inshore.prop.plant
ggsave("figures/Walak_Inshore.prop.plantEAAn.pdf", width=5, height=7)
```
